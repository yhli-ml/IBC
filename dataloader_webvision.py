...
class imagenet_dataset(Dataset):
    def __init__(self, root_dir, transform, num_class):
        self.root = os.path.join(root_dir, 'WebVision-50', 'imagenet', 'val')
        self.transform = transform
        self.val_data = []
        for c in range(num_class):
            imgs = os.listdir(os.path.join(self.root,str(c)))
            for img in imgs:
...
class webvision_dataset(Dataset):
    def __init__(self, root_dir, transform, mode, num_class, pred=[], probability=[], refine_labels=None, imb_type='exp', imb_factor=1):
        self.root = os.path.join(root_dir, 'WebVision-50')
        self.transform = transform
        self.mode = mode
...
class webvision_dataloader():
    def __init__(self, batch_size, num_class, num_workers, root_dir, imb_ratio=1):
    ...
    def run(self,mode,pred=[],prob=[],refine_labels=None, imb_factor=1):
        if mode=='warmup':
            ...
            labeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="labeled",num_class=self.num_class,pred=pred,probability=prob, refine_labels=refine_labels, imb_factor=self.imb_factor)
            ...
            unlabeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="unlabeled",num_class=self.num_class,pred=pred, imb_factor=self.imb_factor)
