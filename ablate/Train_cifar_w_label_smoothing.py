...
parser.add_argument('--data_path', default='/nas/datasets/cifar-10-batches-py', type=str, help='path to dataset')
...
parser.add_argument('--k_per', default=0.5, type=float)
parser.add_argument('--epsilon', default=0.1, type=float)

file_name = os.path.splitext(os.path.basename(__file__))[0]
args = parser.parse_args()
if args.dataset == 'cifar10':
    args.data_path="/nas/datasets/cifar-10-batches-py"
elif args.dataset == 'cifar100':
    args.data_path="/nas/datasets/cifar-100-python"
...
# Training
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, current_labels, tmp_img_num_list, nearest_neighbors_idx):
    net.train()
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, index_x) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            # Reduce confidence for the true label
            px[range(batch_size), labels_x.argmax(dim=1)] = 1 - args.epsilon + args.epsilon / args.num_class
            # Increase confidence of other labels
            other_labels = torch.arange(batch_size).unsqueeze(1).expand(-1, args.num_class).cuda()  # Create a range of batch_size and k
            mask = other_labels != labels_x.argmax(dim=1).unsqueeze(1)  # Mask to exclude the true labels
            px[mask] += args.epsilon / args.num_class
...
def test(epoch, net1, net2):
    ...
    many_shot_classes = set(range(0, int(args.num_class * 0.3)))
    medium_shot_classes = set(range(int(args.num_class * 0.3), int(args.num_class * 0.7)))
    few_shot_classes = set(range(int(args.num_class * 0.7), args.num_class))
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            outputs1 = net1.classify2(feats1)
            outputs2 = net2.classify2(feats2)
            outputs = outputs1 + outputs02 + outputs01 + outputs02
            _, predicted = torch.max(outputs, 1)            
    ...
    print(f"\n| Test Epoch #{epoch}\t Accuracy: {acc:6.2f}%")
    print(cls_acc)
    print(f"Many Shot Accuracy: {many_shot_acc:6.2f}%")
    print(f"Medium Shot Accuracy: {medium_shot_acc:6.2f}%")
    print(f"Few Shot Accuracy: {few_shot_acc:6.2f}%")
...
def eval_train(model, cfeats_EMA, cfeats_sq_EMA):
    ...
    k = int(args.num_class * args.k_per)  # Number of nearest neighbors
    ...
    print(f"Epoch {epoch} - Silhouette Coefficient: {silhouette_avg}")
    ...
    for i in range(args.num_class):
        ...
        # knowing true label
        # pred[idx_class[i]] =  noisy_labels[idx_class[i]] == clean_labels[idx_class[i]]
    ...
    print("Precision: %.4f%% %s\n" %(precision.item(), str(cls_prec)))

    # ----------------------------------------------------------------
    # Feature Visualization: Select classes for head, medium, and tail        
    if epoch == warm_up + 1 or epoch % 10 == 0:

        # Classify based on the indices (head, medium, tail)
        sorted_indices = np.arange(args.num_class)
        
        head_classes = [sorted_indices[0], sorted_indices[int(args.num_class * 0.3)]]
        medium_classes = [sorted_indices[int(args.num_class * 0.3)+1], sorted_indices[int(args.num_class * 0.7)]]
        tail_classes = [sorted_indices[int(args.num_class * 0.7)+1], sorted_indices[-1]]

        # Combine selected classes
        selected_classes = np.concatenate([head_classes, medium_classes, tail_classes])

        # Use t-SNE to reduce dimensions to 2D for visualization
        tsne = TSNE(n_components=2, random_state=0)
        features_2d = tsne.fit_transform(normalized_features_np)

        # Plot the feature space scatter plot for the selected classes
        plt.figure(figsize=(8, 6))
        for class_idx in selected_classes:
            class_mask = clean_labels == class_idx
            # Determine the prefix based on the class index
            if class_idx in head_classes:
                label_prefix = 'Head'
            elif class_idx in medium_classes:
                label_prefix = 'Medium'
            else:
                label_prefix = 'Tail'
            plt.scatter(features_2d[class_mask, 0], features_2d[class_mask, 1], label=f'{label_prefix} Class {class_idx}', s=5, alpha=1)

        # Set plot labels and title
        plt.title(f'Epoch {epoch} - Feature Space (Head, Medium, Tail Classes)')
        plt.xlabel('t-SNE Component 1')
        plt.ylabel('t-SNE Component 2')
        plt.legend()

        # Save the plot to a file
        save_name = f'./feature_space_fig/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/{file_name}_ep{epoch}.png'
        os.makedirs(os.path.dirname(save_name), exist_ok=True)
        plt.savefig(save_name)
        plt.close()

    return pred, confs_BS_max, tmp_img_num_list, nearest_neighbors_idx, silhouette_avg
...
if os.path.isfile(args.pretrained):
    ...
    assert set(msg.missing_keys) == {"linear.weight", "linear.bias", "linear2.weight", "linear2.bias"} #
    assert set(msg2.missing_keys) == {"linear.weight", "linear.bias", "linear2.weight", "linear2.bias"} #
...
for epoch in range(start_epoch, args.num_epochs + 1):   
    ...
    if epoch <= warm_up:
        ...
        pred1, prob1, tmp_img_num_list1, nearest_neighbors_idx1, silhouette_avg1 = eval_train(net1, cfeats_EMA1, cfeats_sq_EMA1)  # sample selection from net1
        pred2, prob2, tmp_img_num_list2, nearest_neighbors_idx2, silhouette_avg2 = eval_train(net2, cfeats_EMA2, cfeats_sq_EMA2)  # sample selection from net2

        silhouette_scores.append((silhouette_avg1+silhouette_avg2)/2)

        print('Train Net1')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred2,prob2) # co-divide
        train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, current_labels2, tmp_img_num_list2, nearest_neighbors_idx1) # train net1  
        
        print('\nTrain Net2')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred1,prob1) # co-divide
        train(epoch, net2, net1, optimizer2, labeled_trainloader, unlabeled_trainloader, current_labels1, tmp_img_num_list1, nearest_neighbors_idx2) # train net2      

    test(epoch, net1, net2)

    if epoch in [warm_up, args.num_epochs]:
        save_path = f'./checkpoint/model_{file_name}_{model_name}_e{epoch}.pth'
...
save_name = f'./saved/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/{file_name}_silhouette_scores.pt'
os.makedirs(os.path.dirname(save_name), exist_ok=True)
torch.save(silhouette_scores, save_name)
