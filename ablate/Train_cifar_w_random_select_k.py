...
def eval_train(model, cfeats_EMA, cfeats_sq_EMA):
    ...
    # -----------------------------------------
    def find_knn_with_random(features, prototypes, k):
        ...
        # Step 2: Randomly choose k prototypes for each sample
        num_prototypes = prototypes.shape[0]
        random_indices = np.random.randint(0, num_prototypes, size=(normalized_features.shape[0], k))

        return normalized_features.cpu().numpy().astype(np.float32), random_indices  # Return the random indices of k prototypes for each sample

    k = int(args.num_class * args.k)  # Number of nearest neighbors
    normalized_features_np, nearest_neighbors_idx = find_knn_with_random(total_features, cfeats_EMA, k)
    # -----------------------------------------
