...
class imagenet_lt_dataset(Dataset): 
    def __init__(self, dataset, imb_type, imb_factor, noise_mode, noise_ratio, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        self.classnames_txt = os.path.join(root_dir, "classnames.txt")
        self.train_txt = os.path.join(root_dir, "ImageNet_LT_train.txt")
        self.test_txt = os.path.join(root_dir, "ImageNet_LT_test.txt")
        ...
        if self.mode=='test':
            self.test_data, self.test_label = self.load_data(self.test_txt)                   
        else:    
            self.train_data, self.train_label = self.load_data(self.train_txt)

            self.data = self.train_data
            self.targets = np.array(self.train_label)
            self.cls_num = 1000
            noise_ratio = self.noise_ratio

            self.img_num_list = self.get_img_num_per_cls(self.cls_num, imb_type, 256)
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    print('Numer of labeled samples:%d  AUC:%.4f '%(pred.sum(),auc))
...
class imagenet_lt_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            all_dataset = imagenet_lt_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_train, mode="all",noise_file=self.noise_file)                
            ...
            labeled_dataset = imagenet_lt_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_train, mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)              
            ...
            unlabeled_dataset = imagenet_lt_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_train, mode="unlabeled", noise_file=self.noise_file, pred=pred)                    
            ...
            test_dataset = imagenet_lt_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_test, mode='test')      
            ...
            eval_dataset = imagenet_lt_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_test, mode='all', noise_file=self.noise_file)      
