...
for epoch in range(start_epoch, args.num_epochs + 1):   
    ...
    if epoch > warm_up:
        ...
        save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}_{args.epsilon}_{args.k}/prediction_{epoch}.csv'
        os.makedirs(os.path.dirname(save_name), exist_ok=True)
        df.to_csv(save_name, index=False)

        df = pd.DataFrame(prob2)
        save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}_{args.epsilon}_{args.k}/probability_{epoch}.csv'
        os.makedirs(os.path.dirname(save_name), exist_ok=True)
        df.to_csv(save_name, index=False)
