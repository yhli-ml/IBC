...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, imb_type, imb_factor, noise_mode, noise_ratio, root_dir, transform, mode, noise_file='', pred=[], probability=[]): 
        ...
        if self.mode=='test':
            ...
            os.makedirs(os.path.join(file_path, 'imb_file'), exist_ok=True)
            imb_file = os.path.join(file_path, 'imb_file', 'cifar' + str(self.cls_num) + '_' + imb_type + '_' + str(imb_factor))
            self.gen_imbalanced_data(self.img_num_list, imb_file)
            
            os.makedirs(os.path.join(file_path, 'noise_file'), exist_ok=True)
            noise_file = os.path.join(file_path, 'noise_file', 'cifar' + str(self.cls_num) + '_' + imb_type + '_' + str(imb_factor) + '_' + noise_mode + '_' + str(noise_ratio))
...
class cifar_dataloader():  
    def __init__(self, dataset, imb_type, imb_factor, noise_mode, noise_ratio, batch_size, num_workers, root_dir, noise_file=''):
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, imb_type=self.imb_type, imb_factor=self.imb_factor, noise_mode=self.noise_mode, noise_ratio=self.noise_ratio, root_dir=self.root_dir, transform=self.transform_train, mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob)              
