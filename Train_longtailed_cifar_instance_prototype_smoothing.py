...
class InstancePrototypeLabelSmoothing:
    def __init__(self, num_classes, max_smoothing=0.2, min_smoothing=0.05, similarity_temp=0.5, update_freq=5):
        """
        Instance-to-Prototype Semantic Label Smoothing based on feature-space similarity.
        
        Instead of computing prototype-to-prototype similarities, this approach computes
        instance-to-prototype similarities for more fine-grained and personalized label smoothing.
        ...
        print(f"Initialized Instance-Prototype Label Smoothing with max_smoothing={max_smoothing}, "
    ...
    def update_class_prototypes(self, model, dataloader, current_epoch):
        ...
        print(f"Updating class prototypes for instance-prototype label smoothing at epoch {current_epoch}...")
        model.eval()
    ...
    def compute_instance_prototype_similarities(self, features):
        ...
        Args:
            features: Instance features (batch_size, feature_dim)
            
        Returns:
            similarities: Similarity matrix (batch_size, num_classes)
        """
        # Normalize features if they aren't already
        features = F.normalize(features, p=2, dim=1)
        
        # Compute cosine similarity between instances and prototypes
        similarities = torch.mm(features, self.class_prototypes.t())
    ...
    def __call__(self, features, labels, num_classes, class_types, smoothing=None):
        ...
        Returns:
            smooth_labels: Instance-specific semantically smoothed targets (batch_size, num_classes)
        """
        batch_size = labels.size(0)
        
        # If smoothing is 0 or prototypes aren't computed yet, return one-hot labels
        if smoothing == 0 or self.class_prototypes is None:
            one_hot = torch.zeros(batch_size, num_classes, device=labels.device)
            one_hot.scatter_(1, labels.unsqueeze(1), 1)
            return one_hot
        
        # Use provided smoothing value or default
...
def train(epoch, net, optimizer, trainloader, instance_prototype_smoother):
    ...
    # Update class prototypes if using instance-prototype smoothing
    if epoch > args.warm_up:
        instance_prototype_smoother.update_class_prototypes(net, trainloader, epoch)
...
# Initialize instance-to-prototype label smoothing
instance_prototype_smoother = InstancePrototypeLabelSmoothing(
...
for epoch in range(start_epoch, args.num_epochs + 1):   
    ...
    if epoch <= warm_up:
        ...
        print('Train Net with Instance-Prototype Label Smoothing')
        train(epoch, net, optimizer, train_loader, instance_prototype_smoother)
