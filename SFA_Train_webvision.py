...
parser.add_argument('--gpuid2', default=4, type=int)
parser.add_argument('--num_class', default=50, type=int)
parser.add_argument('--imb_factor', default=1, type=int)
...
def train(epoch,net,net2,optimizer,labeled_trainloader,unlabeled_trainloader, tmp_img_num_list, device,whichnet):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, index_x) in enumerate(labeled_trainloader):      
        ...
        inputs_x, inputs_x2, labels_x, w_x, index_x= inputs_x.to(device,non_blocking=True), inputs_x2.to(device,non_blocking=True), labels_x.to(device,non_blocking=True), w_x.to(device,non_blocking=True), index_x.to(device,non_blocking=True)
...
if __name__ == "__main__":
    ...
    loader = dataloader.webvision_dataloader(batch_size=args.batch_size,num_class = args.num_class,num_workers=8,root_dir=args.data_path,imb_ratio=args.imb_factor)
    print('| Building net')
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):   
        ...
        if epoch == warm_up or epoch % 10 == 0:
