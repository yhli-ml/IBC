...
def eval_train(model, cfeats_EMA, cfeats_sq_EMA):
    ...
    def find_knn_with_random_sample_index(features, labels_np, k):
        # Step 1: Normalize the features to unit length
        ...
        # Convert features to numpy arrays for FAISS
        normalized_features_np = normalized_features.cpu().numpy().astype(np.float32)

        # Step 2: Create FAISS index for features (finding nearest samples, not prototypes)
        index = faiss.IndexFlatIP(normalized_features.shape[1])  # Use inner product (dot product) for cosine similarity
        index.add(normalized_features_np)  # Add features to the index (using features as "prototypes")

        # Step 3: Search for the k-nearest neighbors based on the samples
        distances, indices = index.search(normalized_features_np, k)  # distances are the inner product, indices are the k nearest samples

        nearest_labels = labels_np[indices]

        return normalized_features_np, nearest_labels  # The indices of the k nearest samples for each feature

    k = int(args.num_class * args.k)  # Number of nearest neighbors
    normalized_features_np, nearest_neighbors_idx = find_knn_with_random_sample_index(total_features, noisy_labels, k)
    # -----------------------------------------
