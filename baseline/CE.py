...
def eval_train(model, cfeats_EMA, cfeats_sq_EMA):    
    ...
    # ----------------------------------------------------------------
    def noirmalize_feature(features):
        ...
        return normalized_features_np  # The indices of the k nearest prototypes for each sample in features
    # Feature Visualization: Select classes for head, medium, and tail        
    if epoch % 50 == 0:
        # Use t-SNE to reduce dimensions to 2D for visualization
        normalized_features_np=noirmalize_feature(total_features)
        from sklearn.manifold import TSNE
        tsne = TSNE(n_components=2, random_state=0)
        features_2d = tsne.fit_transform(normalized_features_np)

        save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/featur_2d_{epoch}.npy'
...
for epoch in range(start_epoch, args.num_epochs + 1):   
    ...
    if epoch <= warm_up:
        ...
        if epoch > warm_up:
            ...
            save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/prediction_{epoch}.csv'
            os.makedirs(os.path.dirname(save_name), exist_ok=True)
            df.to_csv(save_name, index=False)

            df = pd.DataFrame(prob2)
            save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/probability_{epoch}.csv'
            os.makedirs(os.path.dirname(save_name), exist_ok=True)
            df.to_csv(save_name, index=False)
...
save_name = f'./saved/{file_name}/{args.dataset}_{args.noise_ratio}_{args.imb_factor}/Acc.csv'
os.makedirs(os.path.dirname(save_name), exist_ok=True)
