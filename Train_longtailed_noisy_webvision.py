...
for epoch in range(start_epoch, args.num_epochs + 1):   
    ...
    # Log results
    print(f"\n| Test Epoch #{epoch}\t WebVision Acc: {web_acc[0]:.2f}% ({web_acc[1]:.2f}%) \t "
          f"ImageNet Acc: {imagenet_acc[0]:.2f}% ({imagenet_acc[1]:.2f}%)\n")
    test_log.write(f'Epoch:{epoch} \t WebVision Acc: {web_acc[0]:.2f}% ({web_acc[1]:.2f}%) \t '
                  f'ImageNet Acc: {imagenet_acc[0]:.2f}% ({imagenet_acc[1]:.2f}%)\n')
    test_log.flush()
