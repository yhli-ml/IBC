...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, current_labels, 
          tmp_img_num_list, head_nearest_neighbors_idx, medium_nearest_neighbors_idx, tail_nearest_neighbors_idx, cfeats_EMA=None):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, index_x) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            # Label refinement of labeled samples - extract features for distance calculation
            feats_x, outputs_x = net(inputs_x, return_features=True)
            feats_x2, outputs_x2 = net(inputs_x2, return_features=True)
            
            px = (torch.softmax(outputs_x, dim=1) + torch.softmax(outputs_x2, dim=1)) / 2
            
            # Use average features for distance calculation
            batch_features = (feats_x + feats_x2) / 2  # [batch_size, feat_dim]
            ...
            # 2. Increase confidence of the k nearest class prototypes in feature space with distance-based weighting
            # Convert class prototypes to tensor for distance calculation
            if cfeats_EMA is not None:
                class_prototypes = torch.from_numpy(cfeats_EMA).cuda().float()  # [num_classes, feat_dim]
            
            # For tail expert - distance-weighted neighbor selection
            for i in range(batch_size):
                neighbors = tail_nearest_neighbors_idx[index_x[i]]
                num_neighbors = len(neighbors)
                if num_neighbors > 0 and cfeats_EMA is not None:
                    # Calculate cosine similarities between sample feature and neighbor prototypes
                    sample_feature = batch_features[i].unsqueeze(0)  # [1, feat_dim]
                    neighbor_prototypes = class_prototypes[neighbors]  # [num_neighbors, feat_dim]
                    
                    # Normalize features for cosine similarity
                    sample_norm = F.normalize(sample_feature, p=2, dim=1)  # [1, feat_dim]
                    neighbor_norm = F.normalize(neighbor_prototypes, p=2, dim=1)  # [num_neighbors, feat_dim]
                    
                    # Calculate cosine similarities (higher = more similar)
                    similarities = torch.mm(sample_norm, neighbor_norm.t()).squeeze(0)  # [num_neighbors]
                    
                    # Convert similarities to weights using softmax (temperature scaling for sharpening)
                    temperature = 0.1
                    weights = torch.softmax(similarities / temperature, dim=0)
                    
                    # Distance-based weight distribution
                    weighted_epsilon = args.epsilon * weights
                    targets_x_tl[i, neighbors] += weighted_epsilon
                else:
                    # Fallback to uniform distribution if no prototypes available
                    if num_neighbors > 0:
            ...
            # For medium expert - distance-weighted neighbor selection
            for i in range(batch_size):
                ...
                if num_neighbors > 0 and cfeats_EMA is not None:
                    ...
                    # Distance-based weight distribution
                    weighted_epsilon = args.epsilon * weights
                    targets_x_md[i, neighbors] += weighted_epsilon
                else:
                    # Fallback to uniform distribution if no prototypes available
                    if num_neighbors > 0:
...
def eval_train(model, cfeats_EMA, cfeats_sq_EMA):
    ...
    return pred, confs_BS_max, tmp_img_num_list, head_nearest_neighbors_idx, medium_nearest_neighbors_idx, tail_nearest_neighbors_idx, silhouette_avg, cfeats_EMA
...
for epoch in range(start_epoch, args.num_epochs + 1):
    ...
    # Main training phase with co-teaching
        ...
        pred1, prob1, tmp_img_num_list1, head_nearest_neighbors_idx1, medium_nearest_neighbors_idx1, tail_nearest_neighbors_idx1, silhouette_avg1, cfeats_EMA1 = eval_train(net1, cfeats_EMA1, cfeats_sq_EMA1)
        pred2, prob2, tmp_img_num_list2, head_nearest_neighbors_idx2, medium_nearest_neighbors_idx2, tail_nearest_neighbors_idx2, silhouette_avg2, cfeats_EMA2 = eval_train(net2, cfeats_EMA2, cfeats_sq_EMA2)
        ...
        train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, 
              ...
              head_nearest_neighbors_idx1, medium_nearest_neighbors_idx1, tail_nearest_neighbors_idx1, cfeats_EMA1)
        ...
        train(epoch, net2, net1, optimizer2, labeled_trainloader, unlabeled_trainloader, 
              ...
              head_nearest_neighbors_idx2, medium_nearest_neighbors_idx2, tail_nearest_neighbors_idx2, cfeats_EMA2)
